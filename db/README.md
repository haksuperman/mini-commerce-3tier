...
├── docker/
│   └── docker-compose.yml      # 단독 MySQL 8 (모노레포에서 추출)
├── init/
...
### 원본
원본 모노레포 `docker-compose.yml`(`mysql` 서비스)과 `scripts/init.sql` 에서 추출했으며 원본은
수정하지 않았습니다.
...
├── docker/
│   └── docker-compose.yml      # standalone MySQL 8 (extracted from monorepo)
├── init/
...
### Source
Extracted from the original monorepo `docker-compose.yml` (`mysql` service) and
`scripts/init.sql` without modifying the original.
